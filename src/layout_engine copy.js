...
export default function recompute_layout() {
    ...
    function update_op_h_w(op) {
        ...
        while (queue.length > 0) {
            ...
            let c = queue.shift() // take at ix 0 and shift rest one to the left
            ...
            if (c.is_last_in_line) { // normal elbow (inferred elbow, not Python elbow)
                let terminates_at = utils.get_node(c.terminates_at, op.children)
            ...
            // TODO need to block occupancy for the entire row
            // the above doesn't seem quite right. If expanded, we want to block the box and then also the flat line that extends
            // but we don't want the entire thing to get the height of the box. TODO
            // also note if pre-elbow then we need to block the line aligning w the terminates_at node, also one out from c node TODO
            // only blocking if end of line, not blocking the line itself.

            // Let's move the row estimation to outside this loop, then do row-based here. Make row counter for Y ix in python, then use that
            // to go row by row, for each row block out all occ boxes as well as the line itself.
            ...
            // nested by row id
            // TODO row counter ix needs to be from the y calc, not the row determination calc
            let rows = {}
            // op.children.forEach(c => {
            queue.forEach(c => {
                if (!(c.row_counter in rows)) {
                    rows[c.row_counter] = {
                        "nodes":[]
                    }
                };
                rows[c.row_counter].nodes.push(c)
            })

            for (let rid in rows) {
                let row_nodes = rows[rid].nodes
                row_nodes.sort((a,b) => a.x_relative - b.x_relative)
                let from = row_nodes[0].x_relative
                let last_op = row_nodes[row_nodes.length-1]
                ...
                rows[rid].starts_at_x = from
                rows[rid].ends_at_x = until // all the way till terminating node, ie not just row itself

                rows[rid].y_relative = row_nodes[0].y_relative // all have same y_relative
            }



            // TODO these rows can be compiled once for the peer group, not for each child separately
            // Note doesn't work if do all the rows here, have to only do rows for items in the queue

            for (let rid in rows) {
                let r = rows[rid]
                
                let occ = Math.max(...occupancy.slice(r.starts_at_x, r.ends_at_x+1))
                let row_overlap = occ - r.y_relative +1 // all have same y
                if (row_overlap > 0) { 
                    r.nodes.forEach(o => o.y_relative += row_overlap)
                }
            }
